...
export async function GET() {
  try {
    ...
    const existing = await query('SELECT COUNT(*) FROM blogs');
    const count = parseInt(existing.rows[0].count);
    
    if (count > 0) {
      // Clear existing blogs to reseed
    ...
    for (let i = 0; i < 15; i++) {
      const daysAgo = (i + 1) * 35 + Math.floor(Math.random() * 15);
      ...
      // Make each blog unique by adding date or index
